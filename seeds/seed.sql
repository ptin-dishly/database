...
-- ============================================================================
-- 28. MASSIVE DEMO SEEDS FOR MAY 17 AND MAY 25, 2026
-- ============================================================================
DO $$
DECLARE
    est1_id UUID := '22222222-0002-0002-0002-000000000001';
    est2_id UUID := '22222222-0002-0002-0002-000000000002';
    rm1_id UUID := '44444444-0004-0004-0004-000000000001';
    rm2_id UUID := '44444444-0004-0004-0004-000000000002';
    rm3_id UUID := '44444444-0004-0004-0004-000000000003';
    demo_date DATE;
    demo_dates DATE[] := ARRAY['2026-05-17'::DATE, '2026-05-25'::DATE];
...
BEGIN
    -- Delete previous extra orders for these dates to avoid duplication when running 'make seed' multiple times
    DELETE FROM orders WHERE DATE(created_at) = '2026-05-17' OR DATE(created_at) = '2026-05-25';

    -- 1. Insert/Update New Waiters for Est 1 (Ca la Maria)
    FOR i IN 1..4 LOOP
        SELECT id INTO new_user_id FROM users WHERE email = 'waiter_extra_'||i||'@calamaria.cat';
        IF NOT FOUND THEN
            ...
            INSERT INTO waiter_zones (user_id, room_id) VALUES (new_user_id, CASE WHEN i <= 2 THEN rm1_id ELSE rm2_id END);
        END IF;
    END LOOP;
    
    -- 1. Insert/Update New Waiters for Est 2 (El Racó)
    FOR i IN 1..4 LOOP
        SELECT id INTO new_user_id FROM users WHERE email = 'waiter_extra_'||i||'@elraco.cat';
        IF NOT FOUND THEN
            new_user_id := gen_random_uuid();
            INSERT INTO users (id, establishment_id, email, password_hash, name, role, is_active)
            VALUES (new_user_id, est2_id, 'waiter_extra_'||i||'@elraco.cat', 'pwd', names2[i], 'waiter', true);
            INSERT INTO waiter_zones (user_id, room_id) VALUES (new_user_id, rm3_id);
        END IF;
    END LOOP;
    ...
    FOREACH demo_date IN ARRAY demo_dates LOOP
        FOR i IN 1..70 LOOP
            o_id := gen_random_uuid();
