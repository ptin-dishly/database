...
CREATE TABLE IF NOT EXISTS events (
    ...
    description       TEXT,
    -- Combined date and time into one for better timezone handling (Feedback 4)
    ...
    notes             TEXT,
    -- Removed deleted_at to keep consistency with migration 02 (Feedback 3)
...
CREATE TABLE IF NOT EXISTS comensals (
    ...
    name        VARCHAR NOT NULL,
    notes       TEXT,
    -- Removed seat_label to avoid duplication with seats table (Feedback 5)
    is_active   BOOLEAN NOT NULL DEFAULT TRUE,
    created_at  TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
    updated_at  TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
    -- Ensure a seat is only occupied once per event (Feedback 1)
...
CREATE TABLE IF NOT EXISTS comensal_allergens (
    ...
    -- Changed to RESTRICT for food safety (Feedback 2)
